...
search engine และ screen reader ถือว่า `<h1>` คือชื่อของทั้งหน้าเว็บ ให้ใช้เพียงครั้งเดียวเท่านั้น — สำหรับหัวข้อหลัก จากนั้นใช้ `<h2>` สำหรับ section ระดับบนสุด `<h3>` สำหรับ section ย่อย ไล่ตามลำดับไปเรื่อย ๆ การข้ามระดับ (กระโดดจาก `<h1>` ไป `<h3>`) ก็เหมือนการข้ามระดับในลำดับชั้น component ของ Figma — ในทางเทคนิคก็ยังทำงานได้ แต่ทำลายโครงสร้างที่ทั้งคน (และเครื่องมือ) พึ่งพา
</Tip>
...
> 💡 ตัวอย่างจากของจริง
>
> เครื่องมือสามารถ auto-generate สารบัญ (table of contents) จาก heading hierarchy ของหน้าได้โดยตรง — แต่ต้องมี level ที่ถูกต้องและไม่ข้ามเท่านั้น
...
export const quizText = [
  ...
  {
    q: 'element ใดทำให้ข้อความเป็นตัวหนา และบ่งบอกด้วยว่าสำคัญ?',
  ...
  },
  {
    ...
    options: [
      'browser จะแสดง error',
      'ทำลายลำดับชั้นของเอกสารที่ screen reader และ search engine พึ่งพา',
