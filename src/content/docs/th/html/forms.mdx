...
placeholder (`placeholder="Jane Smith"`) ไม่ใช่ label เพราะจะหายไปเมื่อผู้ใช้เริ่มพิมพ์ ทำให้พวกเขาไม่เหลืออะไรเตือนว่า field นี้มีไว้ทำอะไร ให้ใส่ `<label>` ที่มองเห็นได้ไว้เหนือหรือข้าง input ทุกตัวเสมอ นี่คือหนึ่งในความล้มเหลวด้าน accessibility ของฟอร์มที่พบบ่อยที่สุดบนเว็บไซต์จริง — และเป็นหนึ่งในเรื่องที่แก้ง่ายที่สุด ผู้ใช้ของคุณและผู้ตรวจสอบ accessibility จะขอบคุณคุณ
</Tip>
...
> 💡 ตัวอย่างจากของจริง
>
> การใส่ input type ที่ถูกต้อง (`email`, `tel`, `number`) ให้ทุกช่อง — แบบเดียวกับที่ฟอร์ม checkout ของ Stripe ทำ — คือสิ่งที่ทำให้มือถือแสดง keyboard ที่ถูกต้องและ autofill เลขบัตร/อีเมลได้ ([MDN: ชนิดของ `<input>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/input))

export const quizForms = [
  {
    q: 'คุณจะเชื่อม <label> เข้ากับ <input> element ของตัวเองได้อย่างไร?',
    options: [
  ...
  },
  {
    ...
    options: [
      'placeholder ไม่รองรับใน browser สมัยใหม่',
