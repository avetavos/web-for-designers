...
- Setting `position: absolute` on every element instead of letting normal flow do the work, breaking responsive layout entirely
- Not realizing an inline element doesn't accept width/height directly until it becomes `inline-block` or `block`

## The developer's perspective

### 👨‍💻 What developers actually see

Designers see "an element floating here," but developers see that the default display (block/inline) already decides how elements flow next to each other, with no extra code needed.

> 💡 Real-world example
>
> A block-based editor that lets content grow or shrink with the amount of text is just relying on normal flow (stacked block-level elements) as its foundation — no absolute positioning required.
