...
- Forgetting to set `flex-wrap`, causing the layout to overflow on small screens
- Using margin instead of `gap`, making spacing inconsistent when items are added or removed
- Confusing the main axis with the cross axis when `flex-direction` changes

## The developer's perspective

### 👨‍💻 What developers actually see

Designers see "Auto Layout, like in Figma," but developers have to translate every setting (direction, gap, alignment) into its own property, and test it with different content lengths too.

> 💡 Real-world example
>
> Figma's Auto Layout maps onto Flexbox more directly than any other CSS layout method, which is why Flexbox tends to be the first tool developers reach for once a design uses Auto Layout.
