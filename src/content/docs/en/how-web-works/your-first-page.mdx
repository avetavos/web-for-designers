...
- Forgetting `<!DOCTYPE html>`, which makes the browser render in the wrong mode
- Forgetting the viewport meta tag, which makes mobile show a shrunken, unusable layout
- Getting a filename's case wrong (`Index.html` vs `index.html`), which breaks on a case-sensitive server

## The developer's perspective

### 👨‍💻 What developers actually see

Designers see "one file that opens," but developers have to remember the boilerplate every page needs (meta charset, viewport, a title for both SEO and the browser tab).

> 💡 Real-world example
>
> Static site generators and frameworks like Astro and Next.js scaffold this boilerplate into every page's layout automatically, so a developer can't forget it by accident. ([Astro: Project structure](https://docs.astro.build/en/basics/project-structure/))
